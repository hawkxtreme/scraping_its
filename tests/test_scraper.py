...
async def test_scraper_login_success(mock_logger, mock_playwright):
    ...
    # Set up property mock for url that changes after goto
    type(mock_page).url = PropertyMock(return_value="https://login.1c.ru/user/profile")
...
async def test_scraper_login_failure(mock_logger, mock_playwright):
    ...
    # Extract components from fixture
    mock_page = mock_playwright.context.new_page.return_value
    # Set up property mock for url that returns the failure URL
    type(mock_page).url = PropertyMock(return_value="https://login.1c.ru/some_other_page")  # Имитируем неудачный логин
    mock_page.goto = AsyncMock()
    mock_page.fill = AsyncMock()
    mock_page.click = AsyncMock()
    mock_page.wait_for_load_state = AsyncMock()
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_scraper_scrape_final_articles(mock_logger, mock_playwright):
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_scraper_scrape_final_articles_error_handling(mock_logger, mock_playwright):
    """Тест обработки ошибок в scrape_final_articles."""
    articles_to_scrape = [
        {"url": "https://its.1c.ru/db/article1", "title": "Article 1"},
        {"url": "https://its.1c.ru/db/article2", "title": "Article 2"}
    ]
    ...
    mock_frame = MagicMock()
    mock_frame.content = AsyncMock(return_value="<body>Test Content</body>")
    mock_page.frame = MagicMock(side_effect=[mock_frame, PlaywrightError("Frame not found")]) # Имитируем ошибку на второй статье
    mock_page.goto = AsyncMock()
    mock_page.wait_for_load_state = AsyncMock()
    mock_page.close = AsyncMock()
