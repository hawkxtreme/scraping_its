...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_scraper_get_initial_toc_functionality(mock_logger, mock_playwright):
    """Тест функциональности получения оглавления (замена для discover_articles)."""
    test_url = "https://its.1c.ru/db/test"
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_scraper_scrape_single_article_no_frame(mock_logger, mock_playwright):
    """Тест скрапинга статьи, когда фрейм статьи не найден."""
    article_info = {"url": "https://its.1c.ru/db/article1", "title": "Article 1"}
    formats = ['json']
    ...
    with patch("playwright.async_api.async_playwright", return_value=mock_playwright):
        ...
        # Пытаемся скрапинг статью без фрейма
        result = await scraper.scrape_single_article(article_info, formats, 0, 1)
        
        # Проверяем, что ошибка была залогирована
        log_messages = [call_args[0][0] for call_args in mock_logger.call_args_list]
        assert any("ERROR during scraping of Article 1" in msg for msg in log_messages)
        # Возвращаем article_info даже при ошибке
        assert result == article_info
...
async def test_scraper_scrape_single_article(mock_logger, mock_playwright):
    ...
    assert mock_playwright.chromium.connect_over_cdp.called
    # Проверяем, что страницы были созданы и использованы
...
async def test_scraper_scrape_single_article_error_handling(mock_logger, mock_playwright):
    ...
    assert mock_async_playwright_result.start.call_count == 1
    # В случае ошибки, save_article_content может не вызываться
    assert mock_save_article_content.call_count >= 0
